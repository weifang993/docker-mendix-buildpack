...
public class LogoutHandler extends SAMLHandler {
    ...
    public void handleUserLogout(SAMLRequestContext context, IMxRuntimeRequest request)
        ...
        if (sessionInfo == null) {
            ...
            _logNode.debug("Not a SAML user, logging out anyway: " + (session != null ? session.getUser().getName() : "no session"));
            doMendixLogout(context, session);
        } else {
            Metadata metadata = context.getIdpMetadata().getMetadata(sessionInfo.getEntityId());

            org.opensaml.saml2.metadata.Endpoint logoutLocation = metadata.findLogoutEndpoint(Constants.getBindingURIs());
            if (logoutLocation == null) {
                ...
                _logNode.debug("Logging " + session.getUser().getName() + " out at " + logoutLocation.getLocation());
                ...
                org.opensaml.saml2.metadata.Endpoint singoffLocation = metadata.findLogoutEndpoint(Constants.getBindingURIs());
    ...
    public void handleLogoutResponse(SAMLRequestContext context, IMxRuntimeRequest request)
        ...
        _logNode.debug("Received LogoutResponse for request " + requestID);
    ...
    public void handleLogoutRequest(SAMLRequestContext context, IMxRuntimeRequest request) throws SAMLException, CoreException {
        ...
        _logNode.debug("Received LogoutRequest from IDP (" + idpEntityID + "): " + logoutRequest.getID());
